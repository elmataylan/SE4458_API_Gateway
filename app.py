...
# API Gateway code is below

# Defines base URLs for each services, since I didn't deploy my code I left it as localhost
mobile_provider_base_url = "http://localhost:5000"
banking_app_base_url = "http://localhost:5000"

@app.route('/mobileQueryAPI')
def mobile_query_api():
    subscriberNo = request.args.get('subscriberNo')
    month = request.args.get('month')
    response = requests.get(f"{mobile_provider_base_url}/mobileQueryAPI?subscriberNo={subscriberNo}&month={month}")
    return jsonify(response.json()), response.status_code

@app.route('/mobileQueryDetailedAPI')
def mobile_query_detailed_api():
    subscriberNo = request.args.get('subscriberNo')
    month = request.args.get('month')
    page = request.args.get('page', 1)
    page_size = request.args.get('page_size', 10)
    response = requests.get(f"{mobile_provider_base_url}/mobileQueryDetailedAPI?subscriberNo={subscriberNo}&month={month}&page={page}&page_size={page_size}")
    return jsonify(response.json()), response.status_code

@app.route('/BankingAppQueryBillAPI')
def banking_app_query_bill_api():
    subscriberNo = request.args.get('subscriberNo')
    response = requests.get(f"{banking_app_base_url}/BankingAppQueryBillAPI?subscriberNo={subscriberNo}")
    return jsonify(response.json()), response.status_code


@app.route('/WebSitePayBillAPI/<int:subscriberNo>/<int:month>', methods=['PUT'])
def website_pay_bill_api(subscriberNo, month):
    response = requests.put(f"{mobile_provider_base_url}/WebSitePayBillAPI/{subscriberNo}/{month}")
    return jsonify(response.json()), response.status_code

@app.route('/admin_add_bill', methods=['POST'])
def website_admin_add_bill_api():
    data = request.json
    response = requests.post(f"{mobile_provider_base_url}/admin_add_bill", json=data)
    return jsonify(response.json()), response.status_code

## API gateway code ends here
